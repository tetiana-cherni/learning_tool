...
export default function App() {
	const { isAuthenticated, isLoading, error, loginWithRedirect, logout } =
		useAuth0();
	...
	useEffect(() => {
		if (!isLoading && isAuthenticated) {
			const target = localStorage.getItem("postLoginPage") as Page | null;
			if (target) {
	...
	const startQuiz = async (url: string, questionCount: number) => {
		...
		setCurrentPage("quiz");
		const response: QuizResponse | undefined = await fetchQuizFromApi(
			url,
			questionCount,
		);
	...
	const handleViewProfile = () => {
		if (!isAuthenticated) {
			localStorage.setItem("postLoginPage", "profile");
			loginWithRedirect();
		} else {
	...
	return (
		<ThemeProvider>
			<div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 dark:from-gray-900 dark:via-gray-900 dark:to-gray-800">
				...
				<main
					className={`container mx-auto px-4 py-8 ${currentPage === "profile" ? "max-w-6xl" : "max-w-4xl"}`}
				>
					{currentPage === "home" && (
